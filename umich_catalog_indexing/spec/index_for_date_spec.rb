...
RSpec.describe IndexForDate  do 
  before(:each) do 
    @params = {
      ...
      alma_files: [
        ...
        "/some_directory/file2_20220101_delete.tar.gz",
        "/some_directory/file1_20220101_new.tar.gz",
        "/some_directory/file2_20220101_new.tar.gz",
  ...
  context "#run" do
    it "calls IndexIt with appropriate files and solr" do
      ...
      expect(@params[:index_it]).to receive(:perform).with("/some_directory/file2_20220101_new.tar.gz", @params[:solr_url])
      subject.run
    end
    it "calls DeleteIt with appropriate files and solr" do
      ...
      expect(@params[:delete_it]).to receive(:perform).with("/some_directory/file2_20220101_delete.tar.gz", @params[:solr_url])
      subject.run
    end
    ...
    it "does not call IndexIt when no new files" do
      @params[:alma_files][2] = "not_new_anymore"
